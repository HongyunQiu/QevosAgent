...
第一次使用，建议先准备环境变量文件，将.env.example文件转成生产级文件.env：

```bash
cp .env.example .env
```

然后修改.env文件中的相关内容，包括本地模型访问地址，或者在线模型的key等
...
仓库里提供了 `.env.example` 作为环境变量模板。当前 `run_goal.py` 会在启动时自动读取当前目录下的 `.env`，并且遵循“外部环境变量优先于 `.env`”的规则。

需要注意两层“默认值”：

- **代码级 fallback**：如果既没有 `.env`，也没有在 shell 里设置 `OPENAI_PROFILE`，`run_goal.py` 会回退到 `oss120b`
- **模板级默认值**：当前 `.env.example` 里写的是 `OPENAI_PROFILE=qwen3527dgx`，所以如果你直接 `cp .env.example .env`，实际默认会走 `qwen3527dgx`

另外，`.env` 是本地配置文件，默认在 `.gitignore` 中，不会提交到仓库。
...
`.env` 文件支持最常见的 `KEY=VALUE` 形式，也兼容 `export KEY=VALUE`。如果某个变量已经在 shell 里显式设置，`.env` 不会覆盖它。
...
- 还没有 `pyproject.toml`，当前依赖管理以 `requirements.txt` 为主
