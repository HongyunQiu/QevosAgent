...
> **状态**：全文描述的是**当前代码的实际行为**，不是待办计划。
> 执行图四批与时间预算（§7B）四批均已落地。
...
> 时限**只设在图这一层**，由模型自己申请；run 级不设墙钟截止期。
> 图配额到期只关图、自动回自由模式，run 由 7A 的迭代预算与收敛检测终止。
...
设计中途曾有过两级时限：run 级墙钟截止期（`RUN_TIME_BUDGET`）+ 图级工作量配额。
**run 级那一层后来整个取消了**，理由是：

- 开放式任务的总时长根本估不准，估错就是砍掉有用的工作——这正是当初反对
  迭代上限的**同一个理由的时间版**
- 两者的性质不同：**图配额是模型给自己的承诺**，估错代价小（到期只回自由模式，
  任务继续）；**run 时限是对现实的硬承诺**，估错代价大（直接收尾结束）
- 时间感知的价值全部挂在图那一层——投影的时间行、限期取舍、实测速率反哺，
  取消 run 级之后**一点没打折**

| 层 | 时限 | 时钟 | 到期后 |
|---|---|---|---|
| **run** | **不设** | — | 由迭代预算 + 收敛检测终止（与引入时间预算之前一致） |
| **图** | **模型申请**（可不申请），不设上界 | `active`（扣暂停） | `status = expired` → **自动回自由模式，run 继续** |

`active` 对应一份工作量额度："给你 2 小时干活"，人去睡觉的 8 小时不该算。

**图的收口 ≠ 运行时的收口。** 图是插入在运行时中的一段；图跑完，run 未必结束。
到期只关这张图：`expired` 与 `abandoned` **必须区分**——abandoned 是"模型判断这个
分解错了"，expired 是"时间到了但活没干完"，两种完全不同的历史，混在一起地图就说不清。
之后自动回自由模式，模型可以再开一张更小的图、直接收尾、或先处理最要紧的部分。
"一张接一张开图"由收敛检测兜底。
...
| **B** ✅ | 图级 `active` 配额 + `expired` + 自动回自由模式；stall 阈值取先到者 |
| **C** ✅ | 投影的时间两行 + 70%/90% 软触发 + 规划新图前喂速率 |
| **D** ✅ | gaps 重算（产物已存在则不报）+ dashboard 时间条与账本 |

A 是地基，必须先做且单独验证——时间相关的 bug 最难查。

施工中修掉的两个坑，都值得记下来：

- **收尾窗口一开，时间检查不能被永久关闭**。批 B 曾有过 run 级墙钟时限，
  我给条件加了 `not _wrapup_window`，结果窗口开启后时间不再检查、而迭代此时
  又不承担终止职责，run 永远跑下去。冒烟直接挂死才暴露出来。
  （该分支已随 run 级时限一起移除，但这个错误模式值得留档）
...
原计划是在批 B 里让 `_grant_budget` / `topup_budget` 退休，由时间接管。
**实施时否决了**，而 run 级时限取消之后更是如此：迭代预算仍是 run 级唯一的
终止条件之一。
...
| 情形 | 终止条件 |
|---|---|
| 无图 | 迭代预算（与引入时间预算之前完全一致） |
| 图申请了配额 | 到期只关图、回自由模式；run 仍由迭代 + 收敛检测终止 |
| 图没申请配额 | 图不会到期；图上有活时迭代持续补发，靠收敛检测兜 |
...
| L3 求助用户 | L2 之后再停滞 `GRAPH_STALL_L3_EXTRA`（默认 20）轮，**每被打扰一次门槛翻倍** | `ask_user` 暂停 + 落终态 |

`unverified_streak ≥ 5` 作为 L1 的附加软触发，**只提示不阻断**。

`open_fanout` **需要预热**：至少闭合过一个节点才参与判定，理由见 §13。

**L3 打扰要退避。** 用户答完"请继续"（没给新信息）之后续跑会重置梯子，门槛若不变，
20 轮后会原样再问一遍——实战里连问了两次。因此每触发一次 L3，下一次的门槛翻倍，
且该计数**刻意不随续跑重置**（`_graph_l3_count`）。
...
- ⚠ **告诉模型"怎么写"时，措辞必须精确到位置，不能只说"有这个字段"。**
  投影里原先写"可在**工具调用的同一个 JSON 里**附带 `graph_op`"，模型把"同一个 JSON"
  理解成 `args`，于是把 op 塞进了 `args`。改成"与 thought / action / tool / args
  **平级的顶层字段**，绝不能写进 args"，并附一段完整示例 JSON。
  写协议时的自检：这句话有没有第二种读法？只要有，模型迟早会选中另一种
- ⚠ **模型按格式猜错时，绝不能静默失败——要么救回来，要么明确告诉它。**
  上面那次，`parse_response` 读顶层取不到、`executor` 又按 `args_schema` 把它当未知
  参数剔除，**运行时看见了 11 次、每次都默默扔掉**（`meta.ignored_tool_args` 里有记录）。
  模型 11 次推进全部落空，最后自己放弃了图并写下"可能是 graph_op 未正确执行"——
  猜对了根因，却无从确认。现在由 `_rescue_graph_op` 补执行并当场纠正。
  这与 `runtime_patches` 治 JSON 格式错误是同一个思路：格式猜错不该是死路
- ⚠ **有前提的指标要加预热保护**。`open_fanout`（未闭合/已闭合）的说法是"只分叉不
  闭合"，它的前提是**有过闭合的机会**；不加保护的话，一张刚画好的 5 节点计划就是
  5/0 = 5.0，建图后第一轮就被判 L2 停滞。叠加上面那个 bug（永远闭合不掉）之后，
  L2 常驻、每 20 轮升级成 L3 暂停问用户，实战里连打扰了两次（§8）
