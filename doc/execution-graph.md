...
> 唯一例外是 **§7B 时间预算**——设计已定案但尚未动工，该节内有醒目标注。
...
> 本节描述的是**尚未实现**的设计。落地后 7A 的终止职责移交本节，迭代退回纯计量。
...
| 批 | 内容 |
|---|---|
| **A** | 时钟抽象（可注入，测试绝不真 sleep）+ 分类账记账 + `status.json` / `graph.json` 落盘 |
| **B** | run 级 `wall` 时限 + 收尾窗口改由时间触发；图级 `active` 配额 + `expired` 状态 + 自动回自由模式 |
| **C** | 投影的时间两行 + 70%/90% 软触发 + 规划新图前喂速率 |
| **D** | gaps 重算（产物已存在则不报）+ dashboard 时间条与账本 |

A 是地基，必须先做且单独验证——时间相关的 bug 最难查。B 之后行为才真正改变。

连带项：`stall` 阈值改成"**迭代与时间取先到者**"（承认迭代非均匀，
那 `stall_iters ≥ 20` 同样站不住——20 轮可能是 100 秒也可能是 10 小时）；
`_grant_budget` / `topup_budget` / `_add_iterations` 那套在 B 里退休；
