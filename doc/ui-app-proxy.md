# 代理设置对 UI App / 子进程的影响 —— 问题记录（待单独处理）

> 状态：**问题已定性、局部止血，架构级修复待决策**。
...
- [ ] 设置面板配上代理 → 重启实例 → 脚本 App 内 `python -c "urllib POST 127.0.0.1"` 直连成功
- [ ] `.env` 手写 HTTP_PROXY（无 NO_PROXY）→ 同上
- [ ] mechanical_design：加特征 → 模型秒级自动更新（去掉 App 端豁免后仍应通过——豁免可保留作纵深）
- [ ] LLM 出网走代理不受影响（llm.py 原逻辑回归）
