...
## ⑥ 服务端启动插件 — `dashboard/server-plugin.js`

`dashboard/server.js` 在 HTTP 服务 `listen` 成功后调用一次（缺席即 no-op，异常被
try/catch 隔离，绝不影响 dashboard）。用于 PRO 的进程内常驻后台服务（如 Hub 连接器）。

| 导出 | 签名 | 时机 |
|---|---|---|
| `init` | `(ctx) -> void` | 服务 listen 成功后 |

`ctx = { server, serverEvents, wss, getState, broadcast, config }`：
- `server` —— http.Server 实例
- `serverEvents` —— 进程内 EventEmitter
- `wss` —— 浏览器状态流的 WebSocket.Server
- `getState()` —— 返回当前 dashboard `state`（含 runs/status/agentAlive/activeRunId…）
- `broadcast()` —— 向所有 WS 客户端广播当前 state
- `config` —— `{ port, HOST, AGENT_DIR, RUNS_DIR, SKILLS_DIR, APP_VERSION, instanceName }`

插件应自行处理可选启用（如未配置 `HUB_URL` 则直接返回，不发起任何连接）。
...
QevosAgentPro/
  ...
  dashboard/server-plugin.js     ← ⑥（Hub 连接器,进程内常驻）
  agent/pro/hooks.py             ← ④（管理系统实时上报）
  agent/pro/tools.py             ← ⑤（PRO 专属工具）
  agent/pro/__init__.py
  QevosHub/                      ← 管理系统(独立 Node+SQLite 服务,与实例分开部署)
