...
1. **客户端确定性**(拖坐标、连线、删除)→ 面板 `qevos.writeFile('.qevos/view.json' / 'flow.md')`
   → **零 LLM、低延迟**。【✅ 当前】
2. **服务端确定性重计算**(大图布局、批量校验、高清导出)→ 后端原生工具,**不过 LLM**
   (一次性:脚本 App + `QEVOS_RUN_ARGS`;常驻/热路径(持久句柄如相机/CAD 内核):受管 sidecar §7.7)。【◻ 部分】
...
### 例外:运行期真需要常驻进程
按设计仍**没有 App 自己的 server**(§0.5),但常驻逻辑(持久持有 DLL/SDK 句柄)已有正门:
**受管 sidecar(§7.7,✅ 已实现)**——python worker,生命周期归平台,需 frontmatter 显式声明。
SSR / 自带 node server 依旧不支持;绝大多数前端工程 SPA/静态化即可。
...
- **面板截图 → `panel_control(app,"screenshot")`(✅ 已实现)**:桥内 **DOM→PNG**(懒加载 vendored html2canvas,
  ...
  **是重绘非抓屏**(浏览器禁止页面像素级截自己):布局够用、精细样式可能偏差、跨域资源会 taint 失败;
  ...
  > 依赖:html2canvas 是 **vendored 静态资源**(同 xterm),**非 npm 依赖**、不在 package.json、无 `require`。
  > 随构建自动打包(`setup_python.js` 复制 `dashboard/` → `vendor/app/`,electron-builder 带上);
  > **新装开箱即用、老用户更新即得,无需任何 `npm install`**。浏览器模式由 server 直接静态服务。
...
## 7.7 受管 sidecar(常驻 worker:持久持有 DLL/SDK 句柄)✅ 已实现

> 概念/心智模型/调试指南的展开版见 [ui-app-sidecar.md](ui-app-sidecar.md)(开发者向);本节记设计要点。

per-call spawn 在原理上解决不了"持久化运行的库":句柄(相机、CAD 内核)是**进程内存里的东西**,
进程退出即灭,无法序列化复活。答案必然是常驻进程;设计要点是**生命周期归平台**,守住 §0.5
"你不写后端"——App 作者只写 handler 函数,不写 server 骨架(无端口、无部署、无孤儿进程、无鉴权面)。
首个目标场景:qhyccd.dll 相机(OpenCamera 一次,常驻曝光/制冷,禁不起反复 open/close)。

**形态取舍**(为何不是别的):App 自带 HTTP server = 端口冲突 + 孤儿进程 + 本机任意页面可打 + 每人重写骨架;
node 进程内加载 dll = SDK segfault 带走整个 dashboard。sidecar = "标准前后端"的服务端被平台代管后的剩余物。

- **声明**:frontmatter `sidecar: worker.py`(文件必须在 `apps-dist/<id>/` 内,路径穿越被拒);可选
  `sidecar_scope: app|root`(缺省 `app`:全局硬件一进程;`root`:每项目一进程,如 CAD 内核)、
  `sidecar_linger: 秒`(缺省 300,空闲回收延迟)。
- **生命周期**:面板首次 `qevos.call()` 懒启动;每次 call 重置 linger;到期且**无面板打开**(按 bases
  集合查 `panelIsOpen`)才回收——防止"切个页签就重置了制冷半天的相机"。崩溃 → 拒掉 pending + 推
  `{type:'sidecar-exit',code,stderr}` 给面板,下次 call 自动重启。服务退出统一杀。
  保留方法 `$status`/`$stop` 由平台处理、不进 worker(调试/热换 worker 代码用)。
- **控制通道(小件)**:`qevos.call(method,params,{timeout}?)` → `POST /api/app/:id/call` → worker stdin
  一行 `{id,method,params,root}` → stdout 一行 `{id,result|error}`。无 id 的 stdout 行 = worker 主动事件
  `{event,data}` → 经既有 SSE 推给面板(`{type:'sidecar-event',…}`);非 JSON 行按日志忽略(日志走 stderr)。
- **数据通道(大件:帧/模型)不走 RPC**:worker 落盘 base 目录 → 既有 `file-changed` 推送 → 面板
  `readBinary`。深空曝光级帧率绰绰有余;行星视频模式(几十 fps 大帧)若真出现再议专用 WS 二进制通道,现在不做。
- **worker 契约**(skeleton 见 [SKILLS/ui_app.md](../SKILLS/ui_app.md) §4.5):stdin 逐行 JSON、dispatch、
  异常回 `{id,error}` 不退出;**长操作(如曝光)丢线程、立即回 ack、完成后推事件**——单线程阻塞会卡死并发查询。
- runtime 仅 python(ctypes 持 dll 正是目标场景;解释器同脚本 App 的 `PYTHON_CMD`)。env 注入
  `QEVOS_SIDECAR=1 / QEVOS_APP_ID / QEVOS_ROOT`(spawn 时的 base;逐 call 的 root 在每条消息里)。
